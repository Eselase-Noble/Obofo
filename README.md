# WhatsApp Watchdog Agent

Get an **email and/or SMS** the moment a specific person or group **messages or
calls you on WhatsApp** — without opening WhatsApp and without WhatsApp
notifications turned on.

The agent links to your WhatsApp as a companion device (a QR scan, exactly like
WhatsApp Web), quietly watches every incoming message and call, keeps only the
ones from contacts on your **watchlist**, and forwards those to you via Gmail
and Twilio SMS.
...
WhatsApp (your account)
        ...
        ▼
┌─────────────────────┐    match against     ┌──────────────┐
│  Watchdog agent     │───  watchlist.json ──▶│  Notifier    │──▶ 📧 Gmail
│  (this project)     │                       │              │──▶ 📱 Twilio SMS
└─────────────────────┘                       └──────────────┘
```
...
- **`src/notifier.ts`** — sends the alert over email and SMS; a failure in one channel never blocks the other.
...
- **Node.js 18+**
- A **Gmail** account (for email alerts) and/or a **Twilio** account (for SMS)
- A machine that stays **on 24/7** — a Raspberry Pi or a small VPS is ideal. On a
  laptop the agent only catches events while the machine is awake and online.
...
2. Copy your **Account SID**, **Auth Token**, and the Twilio **number** into `.env`.
3. Set `ALERT_SMS_TO` to your personal phone number.
...
A ready-made systemd unit lives in [`deploy/whatsapp-watchdog.service`](deploy/whatsapp-watchdog.service):

```bash
sudo cp deploy/whatsapp-watchdog.service /etc/systemd/system/
sudo systemctl daemon-reload
sudo systemctl enable --now whatsapp-watchdog
journalctl -u whatsapp-watchdog -f   # first run: scan the QR from here
```
